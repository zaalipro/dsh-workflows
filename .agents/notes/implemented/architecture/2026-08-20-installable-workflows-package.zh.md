...
本笔记记录 `@zaalipro/dsh-workflows` 当前已实现的 package 架构。Plugin `0.1.0-rc.6` 只支持官方 Harness `0.1.5-rc.1`；CLI 与 workflow package manifest 必须在 activation 前同时精确匹配。由于 stock evaluator 与 private-directory face 未暴露完整 workflow product contract，本包提供自己的 compatibility evaluator 与 retained-storage descriptor。
...
该设计为严格 compatibility 付出代价：只有官方 `0.1.5-rc.1` 可以加载 plugin `0.1.0-rc.6`，更高版本需要新的 verified package release。Native locking 与 plugin-owned descriptor operation 是必需条件；unsupported platform 会失败，而不是静默弱化 storage。Retained data 与 browser read 有界，因此较旧 terminal detail 可变成明确 truncated 或 evicted 状态。
