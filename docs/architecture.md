...
Current state: plugin `0.1.0-rc.8` is verified against official `0.1.6-alpha.1`. The plugin adapts the public Agent-scoped tool/prompt, filesystem, command, Remote, and provider faces and does not modify the stock `ctx.workflowEngine`.
...
Official `0.1.6-alpha.1` is the only verified installed Host for plugin `0.1.0-rc.8`; `0.1.0-rc.8` is unsupported and later Hosts require re-verification. The compatibility evaluator is package-owned MIT source derived narrowly from the maintained workflow behavior, not a replacement for the process-global stock workflow service.
