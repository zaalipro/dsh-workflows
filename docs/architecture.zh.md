...
本参考说明 `@zaalipro/dsh-workflows` 的已安装架构：它是在 DeepSeek Harness 官方 workflow engine 之上的单一 Host/Client bundle。[用户指南](user-guide.zh.md)负责操作步骤，[测试参考](testing.zh.md)负责发布证据，[架构决策](../.agents/notes/implemented/architecture/2026-08-20-installable-workflows-package.zh.md)负责理由和被拒绝的替代方案。
...
- **Aggregate 与配置**在访问 filesystem 前验证 H，解析每个 Schemastery default，通过 `import.meta.url` 相对路径加载资产，并在 effect ownership 下挂载 child。
...
Terminal transaction 在 head 可见前把 `completionNotice` 从 `none` 变成 `claimed`。一次 bounded enqueue attempt 会把 claim finalize 为 `delivered` 或 `abandoned`；两者都不会重试。每个 cohort 最多携带 20 个 notice 和 262,144 UTF-8 byte，在新 human input 前最多打开三个连续 completion-driven turn。
...
每次 directory walk 都使用 H descriptor-rooted private-directory capability。Root 与 component 必须属于当前 owner，具有严格 `0700`/`0600` mode、预期 type、regular file 单 link、无 symlink 或 junction，以及稳定 device/inode identity。Identity 改变后 cleanup 不会 recurse。永久 kernel lock 没有 PID、heartbeat、stale age、retry、takeover 或 deletion protocol。它协调同一用户下合作的 process，而不是忽略 lease 的 malicious same-UID actor。
...
Exact-Agent tool/prompt shadowing 是官方版本 H 下选定的临时 integration。H 必须提供 identity-checked `ToolRuntime.replace`、`SystemPrompt.replaceSection`、external generated Remote mounting、bounded forwarded event、deferred workflow start 与 quiescent checkpoint、descriptor-rooted storage、client command action 以及 trusted packaged-skill precedence，package activation 才会成功。

官方 commit `141eb6f` 和 tag `dsh-v0.1.0-rc.8` 是 patch-development base，不是兼容的 installed release。Development-fork commit `391c829` 只作为 behavior reference。本包绝不 wholesale copy 其较旧的 RC5-derived file，也绝不提供第二个 engine。
