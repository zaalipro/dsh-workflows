...
当前状态：plugin `0.1.0-rc.9` 已针对官方 `0.1.6-alpha.1` 验证。Plugin 适配 public Agent-scoped tool/prompt、filesystem、command、Remote 与 provider face，不修改 stock `ctx.workflowEngine`。
...
官方 `0.1.6-alpha.1` 是 plugin `0.1.0-rc.9` 唯一已验证的 installed Host；`0.1.0-rc.9` 不受支持，更高 Host 必须重新验证。Compatibility evaluator 是 package-owned MIT source，只窄范围承载 maintained workflow behavior，绝不替换 stock `ctx.workflowEngine` 或 process-global stock workflow service。
