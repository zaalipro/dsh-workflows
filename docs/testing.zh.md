...
Packed consumer 与 CI checkout 官方 DeepSeek Harness `0.1.6-alpha.1` commit `0a15e36e7f82b6ed45af6fa9759f29b40dcd965d`。Package build 与 test 针对 `0.1.6-alpha.1` development dependency 编译；不使用 Harness source patch 或 fork checkout。Package-owned evaluator build、strict NodeNext consumer、add/boot/remove profile cycle 与 removal 后的 stock boot 都是 blocking release evidence。
...
Blocking Ubuntu 24.04 job 运行 Node `22.19.0`、`24` 和 `26`；每个 job 使用 frozen lockfile，并覆盖 build、typecheck、lint、docs、package policy 以及分配的 unit/coverage/snapshot gate。Node 24 还拥有 macOS 14、Windows Server 2022、Chromium helper、race-stress 和 release-pack/packed-consumer job。Packed lane checkout 官方 `0.1.6-alpha.1` commit `0a15e36e7f82b6ed45af6fa9759f29b40dcd965d`；不应用 Harness patch，只 pack 一次并保留一个 digest 和 artifact path，而且要求 Web 与 headless 的 live add/boot/remove。
...
这是 release checklist，不是 coding task、CI step 或 automated Chromium 的替代。只有全部自动化 gate 通过后，才使用安装了完全相同 tested tarball 的真实官方 `0.1.6-alpha.1` Web profile 和 real server/model flow 执行。
