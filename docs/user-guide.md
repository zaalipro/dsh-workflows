...
Use official DeepSeek Harness `0.1.6-alpha.1` with plugin `0.1.0-rc.9`. Ensure `pnpm` is on the service user's `PATH`; `0.1.0-rc.9` and unverified later Harness releases are unsupported.
...
After npm publication, `dsh plugin --profile <profile> add @zaalipro/dsh-workflows@0.1.0-rc.9` is equivalent.
...
Symptom: activation reports an unsupported Host face or version. Install official `0.1.6-alpha.1` with plugin `0.1.0-rc.9`; do not widen the peer range or assume a later Harness version is compatible.
