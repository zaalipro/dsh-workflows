...
使用官方 DeepSeek Harness `0.1.6-alpha.1` 与 plugin `0.1.0-rc.8`。确保 service user 的 `PATH` 中有 `pnpm`；`0.1.0-rc.8` 与尚未验证的更高 Harness release 均不受支持。
...
发布到 npm 后，`dsh plugin --profile <profile> add @zaalipro/dsh-workflows@0.1.0-rc.8` 与之等价。
...
症状：activation 报告不受支持的 Host face 或 version。安装官方 `0.1.6-alpha.1` 与 plugin `0.1.0-rc.8`；不要扩大 peer range，也不要假定更高 Harness version 已兼容。
