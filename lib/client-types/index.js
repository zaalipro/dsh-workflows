...
function commandUiSupportsAction(commandUi) {
    ...
    if (spec === true)
        return true;
    if (typeof spec === 'object' && spec !== null && spec.kind === 'action')
        return true;
...
export function apply(ctx) {
    ...
    root.effect(async () => {
        ...
        if (typeof remote?.$mount !== 'function')
            throw new Error('workflow Remote mount is unavailable');
        ...
        if (typeof remoteOn !== 'function')
            throw new Error('workflow Remote event subscription is unavailable');
