...
const DEFAULTS = {
    ...
    completionCohortMaxItems: 20, completionCohortMaxBytes: 262_144, maxConsecutiveCompletionWakes: 3, saveScope: 'project',
};
const BOOLEAN_FIELDS = ['enabled', 'definitionWatch', 'watchUsePolling'];
 ...
 */
export function resolveWorkflowPackageConfig(input, dshHome) {
    if (typeof input !== 'object' || input === null || Array.isArray(input))
        throw new TypeError('workflow package config must be an object');
    ...
    const result = { ...DEFAULTS, ...input, dshHome: home, runsRoot, ...(bundledDefinitionsDir === undefined ? {} : { bundledDefinitionsDir }) };
    for (const key of BOOLEAN_FIELDS)
        if (typeof result[key] !== 'boolean')
            throw new TypeError(`${key} must be a boolean`);
    if (result.saveScope !== 'project' && result.saveScope !== 'user')
        throw new TypeError('saveScope must be project or user');
    for (const [key, value] of Object.entries(result)) {
        if (key === 'dshHome' || key === 'runsRoot' || key === 'bundledDefinitionsDir' || key === 'saveScope' || BOOLEAN_FIELDS.includes(key))
            continue;
        positiveSafeInteger(key, value);
    }
    for (const [key, ceiling] of Object.entries(FIXED_CEILINGS))
        if (result[key] > ceiling)
            throw new RangeError(`${key} must not exceed ${ceiling}`);
    if (result.defaultAgentBudget > result.maxAgentBudget)
        throw new RangeError('defaultAgentBudget must not exceed maxAgentBudget');
    if (result.maxConcurrentAgents > result.maxAgentBudget)
        throw new RangeError('maxConcurrentAgents must not exceed maxAgentBudget');
    if (result.maxActiveRunsPerSession > result.maxActiveRunsGlobal)
        throw new RangeError('maxActiveRunsPerSession must not exceed maxActiveRunsGlobal');
    if (result.memberOutcomeMaxBytes > result.maxRunDetailsBytes)
        throw new RangeError('memberOutcomeMaxBytes must not exceed maxRunDetailsBytes');
    if (result.maxTerminalResultBytes > result.maxRunDetailsBytes)
        throw new RangeError('maxTerminalResultBytes must not exceed maxRunDetailsBytes');
    if (result.maxLogLineBytes > result.maxLogTotalBytes)
        throw new RangeError('maxLogLineBytes must not exceed maxLogTotalBytes');
    if (result.maxLogTotalBytes > result.maxRunDetailsBytes)
        throw new RangeError('maxLogTotalBytes must not exceed maxRunDetailsBytes');
    if (result.maxRunDetailsBytes > result.maxRunStoreBytes)
        throw new RangeError('maxRunDetailsBytes must not exceed maxRunStoreBytes');
    if (result.maxScriptProjectionBytes > result.maxScriptBytes)
        throw new RangeError('maxScriptProjectionBytes must not exceed maxScriptBytes');
    if (result.scratchMaxPendingOperations > result.scratchMaxOperations)
        throw new RangeError('scratchMaxPendingOperations must not exceed scratchMaxOperations');
    if (result.scratchMaxFileBytes > result.scratchMaxTotalBytes)
        throw new RangeError('scratchMaxFileBytes must not exceed scratchMaxTotalBytes');
    if (result.artifactChunkDefaultBytes > result.artifactChunkMaxBytes)
        throw new RangeError('artifactChunkDefaultBytes must not exceed artifactChunkMaxBytes');
    if (result.remotePageDefault > result.remotePageMax)
        throw new RangeError('remotePageDefault must not exceed remotePageMax');
