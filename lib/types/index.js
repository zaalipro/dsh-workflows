...
export const HOST_COMPATIBILITY = Object.freeze({
    ...
    versions: Object.freeze(['0.1.5-rc.1']),
    evaluator: 'plugin-compat-engine-v1',
});
const INCOMPATIBLE_MESSAGE = '@zaalipro/dsh-workflows 0.1.0-rc.6 supports exactly official DeepSeek Harness 0.1.5-rc.1';
...
function requireFunction(value, member, service) {
    if (typeof value !== 'function') {
        throw new Error(`workflow package requires ${service}.${member} from official DeepSeek Harness 0.1.5-rc.1`);
    }
}
...
/** Required service faces on official dsh 0.1.5-rc.1. */
function assertStockFaces(ctx) {
    ...
    if (typeof optionalProperty(skills, 'registerTrustedPackageSkill') !== 'function'
        ...
        throw new Error('workflow package requires skills.registerProvider on official DeepSeek Harness 0.1.5-rc.1');
    }
...
async function createCompatibilityEngine(ctx, config) {
    ...
    if (subagents === undefined || typeof optionalProperty(subagents, 'getProvider') !== 'function'
        ...
        throw new WorkflowPackageError('official DeepSeek Harness 0.1.5-rc.1 requires the subagents service for plugin workflows', 'WORKFLOW_INCOMPATIBLE_HOST');
    }
...
export async function apply(ctx, input = {}) {
    ...
    try {
        ...
        // Official 0.1.5-rc.1 exposes fs.openPrivateDirectory(), but its production
