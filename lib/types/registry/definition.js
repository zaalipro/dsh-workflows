...
function requireString(value, field, path, nonEmpty = false) {
...
function validateMeta(value, path) {
    if (!record(value))
        fail(path, 'meta must be a plain object');
    const extraMeta = unknownFields(value, META_KEYS);
    if (extraMeta.length > 0)
        fail(path, `unknown metadata field(s) ${extraMeta.join(', ')}`);
 ...
 */
export function parseWorkflowDefinition(bytes, path, scope, expectedName, maxBytes) {
    if (!(bytes instanceof Uint8Array))
        fail(path, 'workflow definition bytes must be a Uint8Array');
    if (!Number.isSafeInteger(maxBytes) || maxBytes < 1)
        fail(path, 'definition byte limit must be a positive safe integer');
    if (bytes.byteLength > maxBytes)
        fail(path, `definition exceeds the ${maxBytes}-byte limit`);
    let raw;
    try {
 ...
 */
export function parseDefinitionFile(raw, path, expectedName) {
    if (typeof raw !== 'string')
        fail(path, 'workflow definition text must be a string');
    let parsed;
    try {
        parsed = JSON.parse(raw);
    }
...
export function validateDefinitionEnvelope(envelope, source = 'workflow definition') {
    if (!record(envelope))
        fail(source, 'a workflow envelope must be a JSON object with { meta, script }');
    const extra = unknownFields(envelope, ENVELOPE_KEYS);
    if (extra.length > 0)
        fail(source, `unknown envelope field(s) ${extra.join(', ')} (expected { meta, script })`);
    if (!own(envelope, 'meta') || !own(envelope, 'script')) {
        fail(source, 'a workflow envelope must be a JSON object with { meta, script }');
    }
    if (typeof envelope.script !== 'string')
        fail(source, 'envelope "script" must be a string');
 ...
 */
export function serializeWorkflowDefinition(envelope) {
...
export function serializeDefinition(envelope) {
...
export function filenameStem(path) {
