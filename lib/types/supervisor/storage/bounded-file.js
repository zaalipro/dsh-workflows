...
export class BoundedFileError extends Error {
    code;
    constructor(message, code = 'WORKFLOW_STORAGE_UNSAFE', options) {
...
function unsafe(path, detail, cause) {
    return new BoundedFileError(`workflow storage path "${path}" is unsafe: ${detail}`, 'WORKFLOW_STORAGE_UNSAFE', cause === undefined ? undefined : { cause });
}
/** Validate a value before it is used as a descriptor-relative component. */
export function assertSafeComponent(value, label = 'path component') {
...
export async function readOwnedRegularFile(directory, name, options) {
...
export async function writeOwnedFileAtomic(directory, name, bytes, options) {
    assertSafeComponent(name, 'file name');
    if (!Number.isSafeInteger(options.maxBytes) || options.maxBytes < 0) {
        throw new RangeError('maxBytes must be a non-negative safe integer');
    }
...
export async function readBoundedUtf8(path, maxBytes) {
    ...
    try {
        ...
        if (typeof process.getuid === 'function' && info.uid !== process.getuid())
            throw unsafe(path, 'file has the wrong owner');
...
export async function writeBoundedAtomic(path, content, options) {
    ...
    if (mode !== 0o600)
        throw unsafe(path, 'workflow storage files must use mode 0600');
        ...
        if (options.createOnly) {
            try {
                ...
                if (error.code === 'EEXIST')
                    throw unsafe(path, 'destination already exists', error);
                throw error;
            }
            await unlink(temp);
        }
