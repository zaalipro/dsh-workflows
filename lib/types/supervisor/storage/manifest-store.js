...
export class FileWorkflowRunStore {
    ...
    async readDetails(runId, request, signal) {
        return this.mutate(async () => {
            ...
            const raw = request.kind === 'members' ? payload.members ?? [] : request.kind === 'logs' ? payload.logs ?? [] : request.kind === 'result' ? payload.result ?? { state: 'not-produced' } : request.kind === 'artifacts' ? payload.artifacts ?? [] : (payload.artifacts ?? []).find((item) => item.name === request.name) ?? null;
