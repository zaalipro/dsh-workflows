 ...
 */
async function inventory(path, root, limit, count, signal) {
    ...
    if (count.value > limit)
        throw unsafe(root, `recovery scan exceeds ${limit} entries`);
    if (!info.isDirectory())
        return;
 ...
 */
async function inventoryDirectoryCapability(directory, root, limit, count, signal) {
    signal?.throwIfAborted();
...
async function inventoryCapabilityLayout(layout, limits, signal) {
    const limit = limits.maxRecoveryEntries;
    if (!Number.isSafeInteger(limit) || limit < 1 || limit > 4_096)
        throw new RangeError('maxRecoveryEntries must be a safe integer from 1 through 4096');
    const count = { value: 0 };
...
export async function inventoryWorkflowStorage(layout, limits, signal) {
    if ('sessions' in layout)
        return inventoryCapabilityLayout(layout, limits, signal);
    const limit = limits.maxRecoveryEntries;
    if (!Number.isSafeInteger(limit) || limit < 1 || limit > 4_096)
        throw new RangeError('maxRecoveryEntries must be a safe integer from 1 through 4096');
    const count = { value: 0 };
    for (const category of ['sessions', 'runs', 'staging', 'quarantine']) {
        ...
        let names;
        try {
            names = (await readdir(categoryPath)).sort();
        }
        catch (error) {
            if (error.code === 'ENOENT')
                continue;
...
export async function recoverWorkflowStorage(layout, store, limits, signal) {
