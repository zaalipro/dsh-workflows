...
export function parseWorkflowToolRequest(value) {
    ...
    if (v.save_scope !== undefined && v.save_scope !== 'project' && v.save_scope !== 'user')
        throw new Error('workflow save_scope must be project or user');
    if (v.resume_from_run_id !== undefined) {
        ...
        for (const key of ['name', 'script', 'script_path', 'meta', 'args', 'validate_only', 'save_scope'])
            if (v[key] !== undefined)
                throw new Error('workflow resume_from_run_id cannot be combined with a source, meta, args, validate_only, or save_scope');
    ...
    if (v.name !== undefined) {
        ...
        if (v.save_scope !== undefined)
            throw new Error('workflow save_scope is allowed only with an inline script validate-only save');
    ...
    if (v.script !== undefined) {
        ...
        if (v.save_scope !== undefined && !validateOnly)
            throw new Error('workflow save_scope is allowed only with an inline script validate-only save');
...
export const WORKFLOW_TOOL_SCHEMA = { type: 'object', additionalProperties: false, properties: { name: { type: 'string' }, script: { type: 'string' }, script_path: { type: 'string' }, meta: { type: 'object' }, args: { type: 'object' }, validate_only: { type: 'boolean' }, save_scope: { type: 'string', enum: ['project', 'user'] }, resume_from_run_id: { type: 'string' }, agent_budget: { type: 'integer', minimum: 1, maximum: 1024 } } };
...
export const WORKFLOW_TOOL_PARAMETERS = {
    ...
    script: {
        type: 'string',
        description: 'The plain-JS workflow script body (top-level await allowed; NO `export const meta` statement). Requires `meta`. Inline script defaults to validate_only (no live children) and saves .dsh/workflows/<meta.name>.workflow.json unless validate_only is false. Commas in object literals, never semicolons.',
    },
    script_path: {
        type: 'string',
