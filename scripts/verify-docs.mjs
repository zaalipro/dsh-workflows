...
const REQUIRED_CONTENT = Object.freeze({
  'README.md': {
    english: [
      ...
      'official DeepSeek Harness **`0.1.5-rc.1`**',
    ...
    chinese: [
      ...
      '官方 DeepSeek Harness **`0.1.5-rc.1`**', '原版 `0.1.0-rc.8`', '不受支持',
      'dsh plugin --profile web add github:zaalipro/dsh-workflows#v0.1.0-rc.6',
      'dsh plugin --profile web add @zaalipro/dsh-workflows@0.1.0-rc.6',
      'dsh plugin --profile headless remove @zaalipro/dsh-workflows',
      'same-process only', '.workflow-storage.lock', 'Interrupted',
  ...
  'docs/architecture.md': {
    english: [
      ...
      'not a security sandbox for hostile code', 'official `0.1.5-rc.1`',
  ...
  '.agents/notes/implemented/architecture/2026-08-20-installable-workflows-package.md': {
    english: [
      ...
      'official Harness `0.1.5-rc.1`', 'compatibility evaluator',
    ],
    chinese: [
      'Status: implemented', '## Summary', '## Context', '## Decision',
      '## Rejected alternatives', '## Consequences', '## References',
      ...
      'Exact-Agent', 'quiescent checkpoint', 'fs-native-extensions@1.5.0',
      'invalidation-only', 'tarball-first',
      '官方 Harness `0.1.5-rc.1`', 'compatibility evaluator',
    ],
  },
})
...
function validateProse(relativePath, text) {
  ...
  if (/(?:\b0\.1\.0-rc\.9\b|\brc[\s._-]*9\b)/iu.test(text)) {
    fail(`${relativePath}: names unverified rc9 as compatible with exact Host 0.1.5-rc.1`)
  }
