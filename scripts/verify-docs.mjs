...
const REQUIRED_CONTENT = Object.freeze({
  'README.md': {
    ...
    chinese: [
      ...
      '官方 DeepSeek Harness **`0.1.6-alpha.1`**', '原版 `0.1.0-rc.12`', '不受支持',
      'dsh plugin --profile web add github:zaalipro/dsh-workflows#v0.1.0-rc.12',
      'dsh plugin --profile web add @zaalipro/dsh-workflows@0.1.0-rc.12',
      'dsh plugin --profile headless remove @zaalipro/dsh-workflows',
      'same-process only', '.workflow-storage.lock', 'Interrupted',
...
function validateProse(relativePath, text) {
  ...
  if (/(?:\b0\.1\.0-rc\.13\b|\brc[\s._-]*13\b)/iu.test(text)) {
    fail(`${relativePath}: names unverified rc13 as compatible with exact Host 0.1.6-alpha.1`)
  }
  if (/stock\s+(?:Harness\s+)?(?:version\s+)?0\.1\.0-rc\.12\s+(?:is|as)\s+compatible/iu.test(text)) {
    fail(`${relativePath}: claims unmodified 0.1.0-rc.12 compatibility`)
  }
}
