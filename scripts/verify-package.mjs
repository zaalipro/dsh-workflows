...
const PACKAGE_VERSION = '0.1.0-rc.11'
...
function verifyManifest(manifest) {
  ...
  expectStringArrayEqual(manifest.dsh.compatibility.versions, ['0.1.6-alpha.1'], 'dsh.compatibility.versions')
...
function verifyDependencies(manifest, clientDeclaration) {
  ...
  for (const name of peersToCheck) {
    ...
    if (name.startsWith('@deepseek-ai/dsh-')) {
      expectEqual(sections.peerDependencies[name], '0.1.6-alpha.1', `peerDependencies.${name}`)
      expectEqual(sections.devDependencies[name], '0.1.6-alpha.1', `devDependencies.${name}`)
    }
  }

  expectEqual(sections.devDependencies['@deepseek-ai/dsh'], '0.1.6-alpha.1', 'devDependencies.@deepseek-ai/dsh')
