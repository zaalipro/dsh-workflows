 ...
 */
export function repairObjectLiteralSemicolons(script: string): string {
  return script.replace(/;\s*(?=(?:["'][^"']*["']|[A-Za-z_$][\w$]*)\s*:)/gu, ', ')
}

function isWouldPause(error: unknown): error is Error {
  return error instanceof Error && error.name === 'WorkflowWouldPause'
}

function gate(_kind: unknown, message: unknown): never {
...
function hostJson(value: unknown): unknown {
...
export async function cannedStockValidate(
  script: string,
  args: unknown = {},
  options: { readonly timeoutMs?: number; readonly filename?: string } = {},
): Promise<
  | { readonly ok: true; readonly status: 'completed'; readonly value: unknown }
  | { readonly ok: true; readonly status: 'would-pause'; readonly value: string }
  | { readonly ok: false; readonly status: 'error'; readonly error: string }
> {
  ...
  const context = createContext({
    agent: async () => null,
    parallel: async (items: unknown) => {
      if (!Array.isArray(items)) return items
      const output: unknown[] = []
      for (const item of items) {
        try {
          if (typeof item === 'function') output.push(await item())
          /* c8 ignore start -- job-map prelude converts objects to thunks */
          else output.push(item)
          /* c8 ignore stop */
        } catch { output.push(null) }
      }
      return output
    },
    pipeline: async (items: unknown, ...stages: Array<(value: unknown, item: unknown, index: number) => unknown>) => {
      const list = Array.isArray(items) ? items : []
      const output: unknown[] = []
      for (let index = 0; index < list.length; index += 1) {
        let value: unknown = list[index]
        try {
          for (const stage of stages) value = await stage(value, list[index], index)
          output.push(value)
        } catch { output.push(null) }
      }
      return output
  ...
  try {
    ...
    if (isWouldPause(error)) return { ok: true, status: 'would-pause', value: error.message }
    return { ok: false, status: 'error', error: asErrorText(error) }
  }
}
