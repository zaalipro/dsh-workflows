...
export type WorkflowCursorKind =
  | 'definitions'
  | 'runs'
  | 'members'
  | 'logs'
...
export type WorkflowCursorDecodeResult =
  | { readonly ok: true; readonly value: WorkflowCursorPayload }
...
const KINDS = new Set<WorkflowCursorKind>(['definitions', 'runs', 'members', 'logs', 'artifacts', 'artifact'])
...
function canonicalJson(value: WorkflowCursorPayload): string {
...
function validPayload(value: unknown): value is WorkflowCursorPayload {
  if (typeof value !== 'object' || value === null || Array.isArray(value)) return false
  const object = value as Record<string, unknown>
  const keys = Object.keys(object)
  if (keys.length !== 7 || !keys.every(key => ['version', 'kind', 'sessionId', 'entityId', 'processEpoch', 'revision', 'offset'].includes(key))) return false
  return object.version === 1
    && typeof object.kind === 'string' && KINDS.has(object.kind as WorkflowCursorKind)
...
function validExpectation(expected: WorkflowCursorExpectation): boolean {
...
function encodeWithSecret(secret: Uint8Array, payload: WorkflowCursorPayload): WorkflowRunCursor {
  secretBytes(secret)
  if (!validPayload(payload)) throw new TypeError('workflow cursor payload is invalid')
 ...
 */
export function encodeWorkflowCursor(secret: Uint8Array, payload: WorkflowCursorPayload): WorkflowRunCursor
export function encodeWorkflowCursor(payload: WorkflowCursorPayload | LegacyCursorPayload): WorkflowRunCursor
export function encodeWorkflowCursor(
  secretOrPayload: Uint8Array | WorkflowCursorPayload | LegacyCursorPayload,
  maybePayload?: WorkflowCursorPayload,
): WorkflowRunCursor {
  if (maybePayload !== undefined) return encodeWithSecret(secretBytes(secretOrPayload), maybePayload)
  const legacy = secretOrPayload as LegacyCursorPayload
  const payload: WorkflowCursorPayload = {
    version: 1,
    kind: legacy.kind ?? (legacy.collection as WorkflowCursorKind),
...
function decodeWithSecret(secret: Uint8Array, cursor: string, expected: WorkflowCursorExpectation): WorkflowCursorDecodeResult {
  try {
    secretBytes(secret)
    if (typeof cursor !== 'string' || cursor.length === 0 || cursor.length > MAX_CURSOR_CHARS) return { ok: false, reason: 'invalid' }
    if (!validExpectation(expected)) return { ok: false, reason: 'invalid' }
    const separator = cursor.indexOf('.')
    if (separator <= 0 || separator !== cursor.lastIndexOf('.')) return { ok: false, reason: 'invalid' }
    ...
    if (!body || !suppliedMac || suppliedMac.byteLength !== 32) return { ok: false, reason: 'invalid' }
    const actualMac = createHmac('sha256', secret).update(bodyText, 'utf8').digest()
    if (!timingSafeEqual(actualMac, Buffer.from(suppliedMac))) return { ok: false, reason: 'invalid' }
    const decodedText = new TextDecoder('utf-8', { fatal: true }).decode(body)
    const parsed = JSON.parse(decodedText) as unknown
    if (!validPayload(parsed) || JSON.stringify(parsed) !== decodedText) return { ok: false, reason: 'invalid' }
    if (parsed.kind !== expected.kind || parsed.sessionId !== expected.sessionId || parsed.entityId !== expected.entityId || parsed.processEpoch !== expected.processEpoch) return { ok: false, reason: 'invalid' }
    if (parsed.offset > expected.total) return { ok: false, reason: 'invalid' }
    if (parsed.revision !== expected.revision) return { ok: false, reason: 'stale' }
...
export function decodeWorkflowCursor(secret: Uint8Array, cursor: string, expected: WorkflowCursorExpectation): WorkflowCursorDecodeResult
/** Compatibility decoder used by the original Remote adapter. */
export function decodeWorkflowCursor(cursor: WorkflowRunCursor, expected: LegacyCursorExpectation): WorkflowCursorPayload
export function decodeWorkflowCursor(
  secretOrCursor: Uint8Array | string,
  cursorOrExpected: string | LegacyCursorExpectation,
  maybeExpected?: WorkflowCursorExpectation,
): WorkflowCursorDecodeResult | WorkflowCursorPayload {
  if (maybeExpected !== undefined) return decodeWithSecret(secretBytes(secretOrCursor), cursorOrExpected as string, maybeExpected)
  const expected = cursorOrExpected as LegacyCursorExpectation
  const result = decodeWithSecret(PROCESS_SECRET, secretOrCursor as string, {
    version: 1,
    kind: (expected.kind ?? expected.collection) as WorkflowCursorKind,
  ...
  if (!result.ok) {
    if (result.reason === 'stale') throw new WorkflowCursorError('workflow page cursor is stale; refresh the collection', 'stale-cursor')
