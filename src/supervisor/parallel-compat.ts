 ...
 */
export function stockBudgetScratchPrelude(options: StockScriptCompatibilityOptions = {}): string {
...
export function unwrapStockCompatibilityResult(value: unknown): StockCompatibilityResult | undefined {
  if (typeof value !== 'object' || value === null || Array.isArray(value)) return undefined
  const record = value as Record<string, unknown>
  if (record[STOCK_RESULT_ENVELOPE] !== true
    || typeof record.scratch !== 'object' || record.scratch === null || Array.isArray(record.scratch)) return undefined
  const scratch: Record<string, string> = Object.create(null) as Record<string, string>
  for (const [name, content] of Object.entries(record.scratch as Record<string, unknown>)) {
    if (!/^[A-Za-z0-9][A-Za-z0-9._-]*$/u.test(name) || typeof content !== 'string') return undefined
...
export function scriptWithJobMapParallel(script: string, options: StockScriptCompatibilityOptions = {}): string {
