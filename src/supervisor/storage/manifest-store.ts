...
import type {
  DetailReadRequest, DetailReadResult, RecoveredRun, WorkflowCompletionNoticeFinalization,
  WorkflowRunCommitRequest, WorkflowRunDetailPayloadV2, WorkflowRunDetailSnapshotV2,
  WorkflowRunArtifactIdentity, WorkflowRunArtifactInventory, WorkflowRunArtifactRead,
  WorkflowRunArtifactRecord, WorkflowRunHeadRecord, WorkflowRunIdentity, WorkflowRunInsertRequest, WorkflowRunStore,
  WorkflowSessionManifest, WorkflowStoreOptions, WorkflowTerminalCommitRequest,
...
const STORAGE_CATEGORIES = ['sessions', 'runs', 'staging', 'quarantine'] as const
...
function artifactIdentityOf(value: { readonly dev: number; readonly ino: number; readonly size: number; readonly mtimeMs: number; readonly ctimeMs?: number; readonly mode?: number; readonly nlink?: number; readonly version?: unknown }): WorkflowRunArtifactIdentity {
  return {
...
function sameArtifactIdentity(left: WorkflowRunArtifactIdentity, right: WorkflowRunArtifactIdentity): boolean {
...
function sameLocalIdentity(left: { readonly dev: number; readonly ino: number; readonly size: number; readonly mtimeMs: number; readonly ctimeMs: number }, right: { readonly dev: number; readonly ino: number; readonly size: number; readonly mtimeMs: number; readonly ctimeMs: number }): boolean {
...
function decodeArtifactWindow(bytes: Uint8Array, maxBytes: number): { readonly text: string; readonly returnedBytes: number } {
  const capacity = Math.min(bytes.byteLength, maxBytes)
  for (let count = capacity; count >= Math.max(0, capacity - 3); count -= 1) {
    try { return { text: utf8Decoder.decode(bytes.subarray(0, count)), returnedBytes: count } }
...
function exposedHead(head: WorkflowRunHeadRecord, runsRoot: string): WorkflowRunHeadRecord {
  return { ...head, scriptPath: join(runsRoot, 'runs', head.runDirectory, 'script.js') }
}
function durableHead(head: WorkflowRunHeadRecord): WorkflowRunHeadRecord {
  const { scriptPath: _scriptPath, ...rest } = head
  return rest
}
function detailLimits(options: WorkflowStoreOptions) {
  return {
...
async function ownerDirectory(path: string, create = false): Promise<void> {
  try {
    const info = await lstat(path)
    if (info.isSymbolicLink() || !info.isDirectory() || (typeof process.getuid === 'function' && info.uid !== process.getuid()) || (info.mode & 0o777) !== 0o700) throw unsafe(path, 'expected owner-only 0700 directory')
  } catch (error) {
    if ((error as NodeJS.ErrnoException).code !== 'ENOENT' || !create) throw error
    await mkdir(path, { recursive: true, mode: 0o700 })
    const info = await lstat(path)
    if (info.isSymbolicLink() || !info.isDirectory() || (info.mode & 0o777) !== 0o700) throw unsafe(path, 'created directory is unsafe')
  }
}
async function ownerFile(path: string, maxBytes: number): Promise<void> {
  const nofollow = (constants as Record<string, number>).O_NOFOLLOW
  if (!nofollow || process.platform === 'win32') throw new BoundedFileError(`safe workflow storage is unavailable on ${process.platform}`, 'WORKFLOW_STORAGE_UNSUPPORTED')
  const handle = await open(path, constants.O_RDONLY | nofollow)
  try {
    const info = await handle.stat()
    if (!info.isFile() || info.nlink !== 1 || (typeof process.getuid === 'function' && info.uid !== process.getuid()) || (info.mode & 0o777) !== 0o600) throw unsafe(path, 'expected owner-only 0600 regular file')
    if (info.size > maxBytes) throw new BoundedFileError(`${path} exceeds ${maxBytes} bytes`, 'WORKFLOW_STORAGE_LIMIT')
  } finally { await handle.close() }
}
async function safeRemoveTree(path: string): Promise<void> {
  const info = await lstat(path)
  if (info.isSymbolicLink()) throw unsafe(path, 'symbolic links are not removable')
  if (info.isDirectory()) {
    ...
    for (const name of (await readdir(path)).sort()) {
      ...
      if (before.isSymbolicLink()) throw unsafe(join(path, name), 'symbolic links are not removable')
      await safeRemoveTree(join(path, name))
    }
    const beforeRemove = await lstat(path)
    if (!beforeRemove.isDirectory() || Number(beforeRemove.dev) !== identity.dev || Number(beforeRemove.ino) !== identity.ino) throw unsafe(path, 'directory identity changed during cleanup')
    await rmdir(path)
  } else {
    if (!info.isFile() || info.nlink !== 1) throw unsafe(path, 'unexpected removable file')
    const current = await lstat(path)
    if (!current.isFile() || Number(current.dev) !== Number(info.dev) || Number(current.ino) !== Number(info.ino) || Number(current.size) !== Number(info.size) || Number(current.mtimeMs) !== Number(info.mtimeMs) || Number(current.ctimeMs) !== Number(info.ctimeMs)) throw unsafe(path, 'file identity changed during cleanup')
    await unlink(path)
  }
}
async function treeBytes(path: string): Promise<number> {
  const info = await lstat(path)
  if (info.isSymbolicLink()) throw unsafe(path, 'symbolic links are not countable')
  if (!info.isDirectory()) return info.size
  let total = 0
  for (const name of await readdir(path)) { assertSafeComponent(name, 'storage entry'); total += await treeBytes(join(path, name)) }
  return total
}
...
async function countEntries(path: string, limit: number, seen: { value: number }): Promise<void> {
  const info = await lstat(path)
  if (info.isSymbolicLink()) throw unsafe(path, 'symbolic links are not allowed during recovery')
  seen.value += 1
  if (seen.value > limit || !info.isDirectory()) return
  for (const name of (await readdir(path)).sort()) {
    assertSafeComponent(name, 'storage entry')
    await countEntries(join(path, name), limit, seen)
    if (seen.value > limit) return
  }
}

async function countCapabilityEntries(directory: PrivateDirectory, limit: number, seen: { value: number }, signal?: AbortSignal): Promise<void> {
  ...
  for (const entry of entries) {
    ...
    seen.value += 1
    if (seen.value > limit) throw new BoundedFileError(`workflow storage path "${directory.path}" is unsafe: recovery scan exceeds ${limit} entries`, 'WORKFLOW_STORAGE_UNSAFE')
    if (entry.type === 'symlink') throw unsafe(directory.path, `symbolic link encountered at "${entry.name}"`)
    if (entry.type === 'directory') {
      const child = await directory.openDirectory(entry.name, signal, { create: false })
      try { await countCapabilityEntries(child, limit, seen, signal) } finally { await child.close().catch(() => undefined) }
    }
  }
}
...
interface RecoveryInventory {
  ...
  readonly manifests: readonly WorkflowSessionManifest[]
  readonly runDirectories: ReadonlySet<string>
  /** All decoded detail snapshots, including snapshots which are not the
   * current manifest reference.  Keeping this map local to the validation
   * pass is important: a corrupt orphan must fail before a row is changed. */
  readonly details: ReadonlyMap<string, WorkflowRunDetailSnapshotV2>
  readonly detailDigests: ReadonlyMap<string, { readonly bytes: number; readonly sha256: string }>
}

function recoveryLimitError(root: string, limit: number): BoundedFileError {
  return unsafe(root, `recovery scan exceeds ${limit} entries`)
}

function isSafeCapabilityIdentity(identity: unknown): identity is { readonly size: number; readonly nlink: number; readonly dev: number; readonly ino: number; readonly version?: unknown } {
  if (typeof identity !== 'object' || identity === null) return false
  const value = identity as Record<string, unknown>
  const size = Number(value.size)
  const nlink = Number(value.nlink)
  if (!Number.isSafeInteger(size) || size < 0 || nlink !== 1) return false
...
async function inspectLocalTree(
  path: string,
  root: string,
  limit: number,
  count: { value: number },
  ...
  count.value += 1
  if (count.value > limit) throw recoveryLimitError(root, limit)
  if (info.isDirectory()) {
  ...
  if (!info.isFile()) throw unsafe(root, `unexpected entry type at "${path}"`)
...
async function inspectCapabilityTree(
  ...
  signal?.throwIfAborted()
  if (directory.listEntries === undefined) throw new BoundedFileError('descriptor-rooted recovery listing is unavailable', 'WORKFLOW_STORAGE_UNSUPPORTED')
  ...
  for (const entry of entries) {
    ...
    if (entry.type === 'directory') {
      const child = await directory.openDirectory(entry.name, signal, { create: false })
      try { total += await inspectCapabilityTree(child, root, limit, count, maxFileBytes, signal) }
    ...
    if (!isSafeCapabilityIdentity(identity)) throw new BoundedFileError(`descriptor-rooted recovery metadata for "${entry.name}" is unavailable`, 'WORKFLOW_STORAGE_UNSUPPORTED')
    if (Number(identity.size) > maxFileBytes) throw new BoundedFileError(`descriptor-rooted file "${entry.name}" exceeds ${maxFileBytes} bytes`, 'WORKFLOW_STORAGE_LIMIT')
...
function allocateDisplayIdentity(manifest: WorkflowSessionManifest, name: string): { readonly ordinal: number; readonly displayName: string } {
  ...
  for (;;) {
    ...
    const collision = manifest.ordinals.some(entry => {
      try { return displayOrdinal(entry.name, displayName) < entry.next }
      catch { return false }
    })
    if (!collision) return { ordinal, displayName }
    ordinal += 1
  }
}
...
export class FileWorkflowRunStore implements WorkflowRunStore {
  ...
  constructor(
    ...
    private readonly lease?: { assertCurrent(): Promise<void> },
    private readonly layout?: WorkflowStorageLayout,
  ) {
    for (const [name, value] of Object.entries(options)) {
      if (name !== 'runsRoot' && value !== undefined && (!Number.isSafeInteger(value) || Number(value) <= 0)) throw new RangeError(`${name} must be a positive safe integer`)
    }
    if (options.maxManifestBytes > 8_388_608 || options.maxRunDetailsBytes > 33_554_432 || options.maxRunStoreBytes > 536_870_912 || options.maxRecoveryEntries > 4_096) throw new RangeError('workflow storage limits exceed fixed ceilings')
  }
  ...
  private async guard(signal?: AbortSignal): Promise<void> {
    ...
    signal?.throwIfAborted(); await this.lease?.assertCurrent(); signal?.throwIfAborted()
    if (this.layout !== undefined) {
      await this.layout.root.assertIdentity(signal)
      for (const directory of [this.layout.sessions, this.layout.runs, this.layout.staging, this.layout.quarantine]) await directory.assertIdentity(signal)
    }
  }
  ...
  private empty(sessionId: string): WorkflowSessionManifest { return { version: 2, sessionId, revision: 0, nextOrdinal: 1, ordinals: [], heads: [] } }
  ...
  private async withDirectory<T>(parts: readonly string[], operation: (directory: PrivateDirectory) => Promise<T>, create = false, signal?: AbortSignal): Promise<T> {
    if (this.layout === undefined) throw new BoundedFileError('descriptor-rooted workflow storage is unavailable', 'WORKFLOW_STORAGE_UNSUPPORTED')
    let directory: PrivateDirectory = this.layout.root
    ...
    try {
      ...
      for (const child of opened.reverse()) await child.close().catch(() => undefined)
    }
  }

  private async withRunDirectory<T>(runDirectory: string, operation: (directory: PrivateDirectory) => Promise<T>, signal?: AbortSignal): Promise<T> {
    assertSafeComponent(runDirectory, 'run directory')
    if (this.layout === undefined) throw new BoundedFileError('descriptor-rooted workflow storage is unavailable', 'WORKFLOW_STORAGE_UNSUPPORTED')
    return this.withDirectory(['runs', runDirectory], operation, false, signal)
  }

  private async withCategory<T>(category: 'sessions' | 'runs' | 'staging' | 'quarantine', operation: (directory: PrivateDirectory) => Promise<T>): Promise<T> {
    if (this.layout === undefined) throw new BoundedFileError('descriptor-rooted workflow storage is unavailable', 'WORKFLOW_STORAGE_UNSUPPORTED')
    const directory = this.layout[category]
    await directory.assertIdentity()
    try { return await operation(directory) } finally { await directory.assertIdentity().catch(() => undefined) }
  }
  ...
  private async removeCapabilityTree(category: 'runs' | 'staging' | 'quarantine', name: string, signal?: AbortSignal): Promise<void> {
    if (this.layout === undefined) throw new BoundedFileError('descriptor-rooted workflow cleanup is unavailable', 'WORKFLOW_STORAGE_UNSUPPORTED')
    assertSafeComponent(name, 'storage entry')
    ...
    try {
      ...
      if (entries === undefined) throw new BoundedFileError('descriptor-rooted workflow cleanup listing is unavailable', 'WORKFLOW_STORAGE_UNSUPPORTED')
      for (const entry of entries) {
        signal?.throwIfAborted(); assertSafeComponent(entry.name, 'storage entry')
        if (entry.type === 'file') {
          if (child.removeFile === undefined) throw new BoundedFileError('descriptor-rooted workflow file cleanup is unavailable', 'WORKFLOW_STORAGE_UNSUPPORTED')
        ...
        } else throw unsafe(this.options.runsRoot, `unsafe entry ${entry.name} during cleanup`)
      }
    } finally { await child.close().catch(() => undefined) }
    if (parent.removeDirectory === undefined) throw new BoundedFileError('descriptor-rooted workflow directory cleanup is unavailable', 'WORKFLOW_STORAGE_UNSUPPORTED')
    await parent.removeDirectory(name, undefined, signal)
    await parent.assertIdentity(signal)
  }

  private async removeCapabilitySubtree(parent: PrivateDirectory, name: string, signal?: AbortSignal): Promise<void> {
    assertSafeComponent(name, 'storage entry')
    const child = await parent.openDirectory(name, signal, { create: false })
    try {
      const entries = child.listEntries === undefined ? undefined : await child.listEntries(signal)
      ...
      for (const entry of entries) {
        ...
        if (entry.type === 'file') {
          if (child.removeFile === undefined) throw new BoundedFileError('descriptor-rooted workflow file cleanup is unavailable', 'WORKFLOW_STORAGE_UNSUPPORTED')
          await child.removeFile(entry.name, entry.identity, signal)
        } else if (entry.type === 'directory') await this.removeCapabilitySubtree(child, entry.name, signal)
        else throw unsafe(this.options.runsRoot, `unsafe entry ${entry.name} during cleanup`)
      }
    } finally { await child.close().catch(() => undefined) }
    if (parent.removeDirectory === undefined) throw new BoundedFileError('descriptor-rooted workflow directory cleanup is unavailable', 'WORKFLOW_STORAGE_UNSUPPORTED')
  ...
  private async capabilityTreeBytes(directory: PrivateDirectory, signal?: AbortSignal): Promise<number> {
    const entries = directory.listEntries === undefined ? undefined : await directory.listEntries(signal)
    if (entries === undefined) throw new BoundedFileError('descriptor-rooted storage accounting is unavailable', 'WORKFLOW_STORAGE_UNSUPPORTED')
    let total = 0
    for (const entry of entries) {
      signal?.throwIfAborted(); assertSafeComponent(entry.name, 'storage entry')
      if (entry.type === 'file') {
        ...
        const child = await directory.openDirectory(entry.name, signal, { create: false })
        try { total += await this.capabilityTreeBytes(child, signal) } finally { await child.close().catch(() => undefined) }
      } else throw unsafe(this.options.runsRoot, `unsafe entry ${entry.name} during accounting`)
    }
    return total
  }

  private async committedBytes(signal?: AbortSignal): Promise<number> {
    if (this.layout === undefined) {
      let total = 0
      for (const category of ['sessions', 'runs', 'staging', 'quarantine']) total += await treeBytes(join(this.options.runsRoot, category))
      return total
    }
    let total = 0
    for (const category of ['sessions', 'runs', 'staging', 'quarantine'] as const) total += await this.capabilityTreeBytes(this.layout[category], signal)
    return total
  }

  private async removeRunTree(runDirectory: string, signal?: AbortSignal): Promise<void> {
    await this.quarantineThenRemove('runs', runDirectory, signal)
  }

  private async removeStagingTree(stageId: string, signal?: AbortSignal): Promise<void> {
  ...
  private async quarantineThenRemove(category: 'runs' | 'staging', name: string, signal?: AbortSignal): Promise<void> {
    assertSafeComponent(name, 'storage entry')
  ...
  private async readCapabilityFile(parts: readonly string[], name: string, maxBytes: number, signal?: AbortSignal): Promise<Uint8Array> {
    return this.withDirectory(parts, directory => directory.readBytes(name, signal, maxBytes), false)
  }

  private async writeCapabilityFile(parts: readonly string[], name: string, content: string, maxBytes: number, createOnly = false, signal?: AbortSignal): Promise<void> {
    const bytes = encoder.encode(content)
    if (bytes.byteLength > maxBytes) throw new BoundedFileError(`workflow storage file exceeds ${maxBytes} bytes`, 'WORKFLOW_STORAGE_LIMIT')
    await this.withDirectory(parts, async directory => {
      ...
      if (directory.fileInfo === undefined && directory.listEntries !== undefined) {
        ...
        if (observed !== undefined) {
          if (observed.type !== 'file') throw unsafe(directory.path, `entry "${name}" is not a regular file`)
          current = observed.identity
        }
      }
      if (createOnly && current !== undefined) throw new BoundedFileError(`private file "${name}" already exists`, 'WORKFLOW_STORAGE_UNSAFE')
      ...
      const expected = current === undefined ? { kind: 'createIfAbsent' as const } : { kind: 'replaceIfVersion' as const, version: current }
      await directory.writeText(name, content, expected, signal)
    }, true)
  }

  private async removeCapabilityFile(parts: readonly string[], name: string, signal?: AbortSignal): Promise<void> {
    if (this.layout === undefined) { await unlink(join(this.options.runsRoot, ...parts, name)).catch(error => { if ((error as NodeJS.ErrnoException).code !== 'ENOENT') throw error }); return }
    await this.withDirectory(parts, async directory => {
      if (directory.removeFile === undefined) throw new BoundedFileError('descriptor-rooted file removal is unavailable', 'WORKFLOW_STORAGE_UNSUPPORTED')
  ...
  private async load(sessionId: string): Promise<WorkflowSessionManifest> {
    ...
    try {
      ...
      if (manifest.sessionId !== sessionId) throw unsafe(this.options.runsRoot, 'Session manifest identity mismatch')
      this.sessions.set(sessionId, clone(manifest)); return clone(manifest)
    } catch (error) {
      if (isNotFound(error)) { const empty = this.empty(sessionId); this.sessions.set(sessionId, empty); return clone(empty) }
      throw error
    }
  }
  private async readDetail(head: WorkflowRunHeadRecord, sessionId: string, cache = true): Promise<WorkflowRunDetailSnapshotV2> {
    const cached = this.details.get(head.runId)
    if (cache && cached?.detailId === head.detail.id) return clone(cached)
    ...
    if (bytes.byteLength !== head.detail.bytes || createHash('sha256').update(bytes).digest('hex') !== head.detail.sha256) throw unsafe(this.options.runsRoot, `detail ${head.detail.id} digest or byte count does not match its manifest head`)
    const detail = decodeWorkflowRunDetails(bytes, path, detailLimits(this.options))
    if (detail.sessionId !== sessionId || detail.runId !== head.runId || detail.runDirectory !== head.runDirectory || detail.detailId !== head.detail.id || detail.snapshotRevision !== head.detail.snapshotRevision) throw unsafe(this.options.runsRoot, 'detail identity does not match its manifest head')
    if (cache) this.details.set(head.runId, clone(detail)); return detail
  }
  ...
  async readRunScript(runDirectory: string, maxBytes: number, signal?: AbortSignal): Promise<string> {
    if (!Number.isSafeInteger(maxBytes) || maxBytes < 0 || maxBytes > descriptorFileCeiling) throw new BoundedFileError('workflow script projection limit is invalid', 'WORKFLOW_STORAGE_LIMIT')
    await this.mutate(async () => {
      await this.guard(signal); await this.ensureInitialized(signal)
    })
    return this.withRunDirectory(runDirectory, async directory => {
      ...
      catch (error) { throw new BoundedFileError('workflow script projection is not valid UTF-8', 'WORKFLOW_STORAGE_CORRUPT', { cause: error }) }
    }, signal)
  }

  /** Inventory scratch files relative to a descriptor-pinned run directory. */
  async listRunArtifacts(runDirectory: string, maxItems: number, signal?: AbortSignal): Promise<WorkflowRunArtifactInventory> {
    if (!Number.isSafeInteger(maxItems) || maxItems < 1) throw new BoundedFileError('workflow artifact limit is invalid', 'WORKFLOW_STORAGE_LIMIT')
    await this.mutate(async () => {
      await this.guard(signal); await this.ensureInitialized(signal)
    })
    return this.withRunDirectory(runDirectory, async directory => {
      ...
      try {
        ...
        const all: WorkflowRunArtifactRecord[] = []
        for (const entry of entries) {
          signal?.throwIfAborted()
          assertSafeComponent(entry.name, 'scratch artifact')
          if (entry.type !== 'file') throw unsafe(this.options.runsRoot, `unsafe scratch artifact ${entry.name}`)
          const identity = entry.identity ?? (scratch.fileInfo === undefined ? undefined : await scratch.fileInfo(entry.name, signal))
          if (identity === undefined) throw new BoundedFileError('descriptor-rooted scratch metadata is unavailable', 'WORKFLOW_STORAGE_UNSUPPORTED')
          if (identity.nlink !== 1) throw unsafe(this.options.runsRoot, `unsafe scratch artifact ${entry.name}`)
          const size = Number(identity.size)
          if (!Number.isSafeInteger(size) || size < 0 || size > descriptorFileCeiling) throw new BoundedFileError(`workflow artifact ${entry.name} exceeds ${descriptorFileCeiling} bytes`, 'WORKFLOW_STORAGE_LIMIT')
      ...
      } finally { await scratch.close().catch(() => undefined) }
    }, signal)
  }
  ...
  async readRunArtifact(
    ...
    offsetBytes: number,
    maxBytes: number,
    expected: WorkflowRunArtifactIdentity,
    signal?: AbortSignal,
  ): Promise<WorkflowRunArtifactRead> {
    assertSafeComponent(name, 'scratch artifact')
    if (!Number.isSafeInteger(offsetBytes) || offsetBytes < 0 || !Number.isSafeInteger(maxBytes) || maxBytes < 1 || maxBytes > 131_072) throw new BoundedFileError('workflow artifact read limit is invalid', 'WORKFLOW_STORAGE_LIMIT')
    await this.mutate(async () => {
      await this.guard(signal); await this.ensureInitialized(signal)
    })
    return this.withRunDirectory(runDirectory, async directory => {
      ...
      try {
        ...
        if (before === undefined || before.nlink !== 1 || before.size < offsetBytes || !sameArtifactIdentity(artifactIdentityOf(before), expected)) throw new BoundedFileError('workflow artifact changed; refresh it before reading', 'WORKFLOW_STALE_REVISION')
        ...
        if (after === undefined || !sameArtifactIdentity(artifactIdentityOf(after), expected)) throw new BoundedFileError('workflow artifact changed while it was being read', 'WORKFLOW_STALE_REVISION')
        return {
  ...
  async readRunReport(runDirectory: string, maxBytes: number, signal?: AbortSignal): Promise<string | undefined> {
    if (!Number.isSafeInteger(maxBytes) || maxBytes < 1 || maxBytes > descriptorFileCeiling) throw new BoundedFileError('workflow report limit is invalid', 'WORKFLOW_STORAGE_LIMIT')
    ...
    return this.withRunDirectory(runDirectory, async directory => {
      ...
      try {
        ...
        if (info.nlink !== 1 || info.size > maxBytes) throw new BoundedFileError('workflow report exceeds its bound', 'WORKFLOW_STORAGE_LIMIT')
  ...
  private async validateRunFiles(head: WorkflowRunHeadRecord, sessionId: string): Promise<void> {
    ...
    if (this.layout !== undefined) {
      await this.withDirectory(['runs', head.runDirectory], async runDirectory => {
        ...
        try {
          if (details.listEntries === undefined) throw new BoundedFileError('descriptor-rooted detail listing is unavailable', 'WORKFLOW_STORAGE_UNSUPPORTED')
          const entries = await details.listEntries()
          let bytes = 0
          for (const entry of entries) {
            if (!/^[a-f0-9]{32}\.json$/u.test(entry.name) || entry.type !== 'file') throw unsafe(this.options.runsRoot, `unsafe detail entry ${entry.name}`)
            const identity = entry.identity ?? (details.fileInfo === undefined ? undefined : await details.fileInfo(entry.name))
            if (identity === undefined) throw new BoundedFileError(`descriptor-rooted detail metadata for "${entry.name}" is unavailable`, 'WORKFLOW_STORAGE_UNSUPPORTED')
            bytes += Number(identity.size)
            if (bytes > this.options.maxRunDetailsBytes) throw new BoundedFileError(`run ${head.runDirectory} details exceed ${this.options.maxRunDetailsBytes} bytes`, 'WORKFLOW_STORAGE_LIMIT')
          }
        } finally {
    ...
    await ownerDirectory(run); await ownerFile(join(run, 'script.js'), 1_048_576); await ownerDirectory(join(run, 'scratch')); await ownerDirectory(join(run, 'details'))
    let bytes = 0
    for (const name of await readdir(join(run, 'details'))) {
      if (!/^[a-f0-9]{32}\.json$/u.test(name)) throw unsafe(this.options.runsRoot, `unsafe detail entry ${name}`)
      const path = join(run, 'details', name); await ownerFile(path, this.options.maxRunDetailsBytes); bytes += (await stat(path)).size
      if (bytes > this.options.maxRunDetailsBytes) throw new BoundedFileError(`run ${head.runDirectory} details exceed ${this.options.maxRunDetailsBytes} bytes`, 'WORKFLOW_STORAGE_LIMIT')
    }
    await this.readDetail(head, sessionId)
  }
  ...
  private async validateLocalRunInventory(
    runDirectory: string,
    inventory: { readonly details: Map<string, WorkflowRunDetailSnapshotV2>; readonly detailDigests: Map<string, { readonly bytes: number; readonly sha256: string }> },
  ): Promise<void> {
    ...
    for (const name of (await readdir(join(run, 'details'))).sort()) {
      ...
      if (!match) throw unsafe(this.options.runsRoot, `unsafe detail entry ${name}`)
      ...
      if (inventory.details.has(key)) throw unsafe(this.options.runsRoot, `detail ${match[1]} is duplicated`)
  ...
  private async capabilityEntries(directory: PrivateDirectory, signal?: AbortSignal): Promise<readonly { readonly name: string; readonly type: string; readonly identity?: any }[]> {
    if (directory.listEntries === undefined) throw new BoundedFileError('descriptor-rooted recovery listing is unavailable', 'WORKFLOW_STORAGE_UNSUPPORTED')
    return directory.listEntries(signal) as any
  }

  private async validateCapabilityRunInventory(
    runDirectory: string,
    inventory: { readonly details: Map<string, WorkflowRunDetailSnapshotV2>; readonly detailDigests: Map<string, { readonly bytes: number; readonly sha256: string }> },
    signal?: AbortSignal,
  ): Promise<void> {
    ...
    try {
      ...
      if (script?.type !== 'file') throw unsafe(this.options.runsRoot, `run ${runDirectory} script is not a regular file`)
      ...
      if (scratchEntry?.type !== 'directory' || detailsEntry?.type !== 'directory') throw unsafe(this.options.runsRoot, `run ${runDirectory} scratch/details layout is unsafe`)
      ...
      try {
        ...
        for (const entry of detailEntries) {
          ...
          if (!match || entry.type !== 'file') throw unsafe(this.options.runsRoot, `unsafe detail entry ${entry.name}`)
          const identity = entry.identity ?? (details.fileInfo === undefined ? undefined : await details.fileInfo(entry.name, signal))
          if (!isSafeCapabilityIdentity(identity)) throw new BoundedFileError(`descriptor-rooted detail metadata for "${entry.name}" is unavailable`, 'WORKFLOW_STORAGE_UNSUPPORTED')
          total += Number(identity.size)
          if (total > this.options.maxRunDetailsBytes) throw new BoundedFileError(`run ${runDirectory} details exceed ${this.options.maxRunDetailsBytes} bytes`, 'WORKFLOW_STORAGE_LIMIT')
          ...
          if (snapshot.runDirectory !== runDirectory || snapshot.detailId !== match[1]) throw unsafe(this.options.runsRoot, `detail ${entry.name} identity does not match run ${runDirectory}`)
          const key = `${runDirectory}/${match[1]}`
          if (inventory.details.has(key)) throw unsafe(this.options.runsRoot, `detail ${match[1]} is duplicated`)
          inventory.details.set(key, snapshot)
          inventory.detailDigests.set(key, { bytes: bytes.byteLength, sha256: createHash('sha256').update(bytes).digest('hex') })
    ...
    } finally { await run.close().catch(() => undefined) }
  }
   ...
   * sidecar. */
  private async validateRecoveryInventory(signal?: AbortSignal): Promise<RecoveryInventory> {
    const count = { value: 0 }
    const details = new Map<string, WorkflowRunDetailSnapshotV2>()
    const detailDigests = new Map<string, { readonly bytes: number; readonly sha256: string }>()
    const manifests: WorkflowSessionManifest[] = []
    ...
    if (this.layout === undefined) {
      for (const category of STORAGE_CATEGORIES) {
        ...
        for (const name of (await readdir(categoryPath)).sort()) {
          ...
          if (!topInfo.isDirectory() || topInfo.isSymbolicLink()) throw unsafe(this.options.runsRoot, `${category} entry ${name} is not a directory`)
          ...
          if (category === 'sessions' && !SESSION_ADDRESS.test(name)) throw unsafe(this.options.runsRoot, `invalid Session directory ${name}`)
          if ((category === 'runs' || category === 'staging' || category === 'quarantine') && !COMPONENT32.test(name)) throw unsafe(this.options.runsRoot, `invalid ${category} entry ${name}`)
        }
      }
      ...
      for (const address of sessionNames) {
        ...
        if (children.length !== 1 || children[0] !== 'manifest.json') throw unsafe(this.options.runsRoot, `Session directory ${address} has an unexpected layout`)
        ...
        if (sessionAddress(manifest.sessionId) !== address) throw unsafe(this.options.runsRoot, `Session directory ${address} does not match its manifest identity`)
      ...
      for (const entry of sessionEntries) {
        if (entry.type !== 'directory' || !SESSION_ADDRESS.test(entry.name)) throw unsafe(this.options.runsRoot, `invalid Session directory ${entry.name}`)
        ...
        try {
          ...
          if (children.length !== 1 || children[0].name !== 'manifest.json' || children[0].type !== 'file') throw unsafe(this.options.runsRoot, `Session directory ${entry.name} has an unexpected layout`)
          ...
          if (sessionAddress(manifest.sessionId) !== entry.name) throw unsafe(this.options.runsRoot, `Session directory ${entry.name} does not match its manifest identity`)
          manifests.push(manifest)
        } finally { await directory.close().catch(() => undefined) }
      }
      const runEntries = await this.capabilityEntries(this.layout.runs, signal)
      for (const entry of runEntries) {
        if (entry.type !== 'directory' || !COMPONENT32.test(entry.name)) throw unsafe(this.options.runsRoot, `invalid run entry ${entry.name}`)
        runDirectories.add(entry.name)
        await this.validateCapabilityRunInventory(entry.name, { details, detailDigests }, signal)
      }
      for (const category of ['staging', 'quarantine'] as const) {
        for (const entry of await this.capabilityEntries(this.layout[category], signal)) {
          if (!COMPONENT32.test(entry.name)) throw unsafe(this.options.runsRoot, `invalid ${category} entry ${entry.name}`)
        }
      }
    }
    ...
    for (const manifest of manifests) {
      for (const head of manifest.heads) {
        if (!runDirectories.has(head.runDirectory)) throw unsafe(this.options.runsRoot, `run directory ${head.runDirectory} is missing`)
        if (referenced.has(head.runDirectory)) throw unsafe(this.options.runsRoot, `run directory ${head.runDirectory} is referenced more than once`)
        referenced.add(head.runDirectory)
        const key = `${head.runDirectory}/${head.detail.id}`
        const detail = details.get(key)
        if (detail === undefined) throw unsafe(this.options.runsRoot, `detail ${head.detail.id} is missing`)
        if (referencedDetails.has(key)) throw unsafe(this.options.runsRoot, `detail ${head.detail.id} is referenced more than once`)
    ...
    if (count.value > this.options.maxRecoveryEntries) throw recoveryLimitError(this.options.runsRoot, this.options.maxRecoveryEntries)
  ...
  private async reconcileRecoveryResidue(inventory: RecoveryInventory, signal?: AbortSignal): Promise<void> {
    const referencedRuns = new Set<string>()
    const referencedDetails = new Set<string>()
    for (const manifest of inventory.manifests) for (const head of manifest.heads) {
    ...
    if (this.layout === undefined) {
      for (const runDirectory of [...inventory.runDirectories].sort()) {
        ...
        for (const name of (await readdir(detailsPath)).sort()) {
          const match = name.match(DETAIL_FILE)
          if (!match) throw unsafe(this.options.runsRoot, `unsafe detail entry ${name}`)
          if (referencedDetails.has(`${runDirectory}/${match[1]}`)) continue
          const path = join(detailsPath, name)
          const before = await lstat(path)
          if (!before.isFile() || before.isSymbolicLink() || before.nlink !== 1) throw unsafe(this.options.runsRoot, `unsafe detail entry ${name}`)
          const current = await lstat(path)
          if (!sameLocalIdentity(before, current)) throw unsafe(this.options.runsRoot, `detail ${name} identity changed during recovery`)
          await unlink(path)
        }
      }
      ...
      try {
        ...
        try {
          for (const entry of await this.capabilityEntries(details, signal)) {
            const match = entry.name.match(DETAIL_FILE)
            if (!match) throw unsafe(this.options.runsRoot, `unsafe detail entry ${entry.name}`)
            if (!referencedDetails.has(`${runDirectory}/${match[1]}`)) await this.removeCapabilityFile(['runs', runDirectory, 'details'], entry.name, signal)
          }
        } finally { await details.close().catch(() => undefined) }
      } finally { await run.close().catch(() => undefined) }
    }
    for (const entry of await this.capabilityEntries(this.layout.staging, signal)) {
      if (entry.type !== 'directory') throw unsafe(this.options.runsRoot, `staging entry ${entry.name} is not a directory`)
    ...
    for (const entry of await this.capabilityEntries(this.layout.quarantine, signal)) {
      if (entry.type !== 'directory') throw unsafe(this.options.runsRoot, `quarantine entry ${entry.name} is not a directory`)
      await this.removeCapabilityTree('quarantine', entry.name, signal)
    }
  }

  private async initializeUnlocked(signal?: AbortSignal): Promise<readonly RecoveredRun[]> {
    await this.guard(signal)
    if (this.initialized) return this.recovered
    if (this.layout === undefined) {
      await ownerDirectory(this.options.runsRoot, true)
      for (const category of STORAGE_CATEGORIES) await ownerDirectory(join(this.options.runsRoot, category), true)
    } else {
      await this.layout.root.assertIdentity(signal)
      for (const category of STORAGE_CATEGORIES) await this.layout[category].assertIdentity(signal)
    }
    ...
    this.sessions.clear(); this.details.clear()
    for (const manifest of inventory.manifests) this.sessions.set(manifest.sessionId, clone(manifest))
    for (const manifest of inventory.manifests) for (const head of manifest.heads) {
      const snapshot = inventory.details.get(`${head.runDirectory}/${head.detail.id}`)
      if (snapshot !== undefined) this.details.set(head.runId, clone(snapshot))
    }
    ...
    const plans: Array<{ readonly manifest: WorkflowSessionManifest; readonly heads: readonly WorkflowRunHeadRecord[]; readonly replacements: readonly { readonly old: WorkflowRunHeadRecord; readonly snapshot: WorkflowRunDetailSnapshotV2; readonly bytes: Uint8Array }[] }> = []
    ...
    for (const manifest of [...this.sessions.values()].sort((left, right) => left.sessionId.localeCompare(right.sessionId))) {
      const heads: WorkflowRunHeadRecord[] = []
      const replacements: { old: WorkflowRunHeadRecord; snapshot: WorkflowRunDetailSnapshotV2; bytes: Uint8Array }[] = []
      for (const original of manifest.heads) {
        ...
        if (ACTIVE.has(original.status)) {
          ...
          if (detail === undefined) throw unsafe(this.options.runsRoot, `detail ${original.detail.id} is missing during recovery planning`)
          const payload: WorkflowRunDetailPayloadV2 = {
            ...detail.payload,
            members: (detail.payload.members ?? []).map(member => member.status === 'running'
              ? { ...member, status: 'cancelled' as const, outcome: member.outcome === 'pending' ? 'not-produced' as const : member.outcome, settledAt: recoveryTime }
              : member),
          }
          const replacement: WorkflowRunDetailSnapshotV2 = { ...detail, detailId: id(), snapshotRevision: detail.snapshotRevision + 1, payload }
    ...
    for (const plan of plans) {
      ...
      for (const replacement of plan.replacements) {
        ...
        if (this.layout !== undefined) await this.writeCapabilityFile(['runs', replacement.old.runDirectory, 'details'], `${replacement.snapshot.detailId}.json`, text, this.options.maxRunDetailsBytes, true, signal)
        else await writeBoundedAtomic(join(this.runPath(replacement.old.runDirectory), 'details', `${replacement.snapshot.detailId}.json`), text, { maxBytes: this.options.maxRunDetailsBytes, createOnly: true })
      }
  ...
  async initialize(signal?: AbortSignal): Promise<readonly RecoveredRun[]> { return this.mutate(() => this.initializeUnlocked(signal)) }
  private async ensureInitialized(signal?: AbortSignal): Promise<void> { if (!this.initialized) await this.initializeUnlocked(signal) }

  private async evictForSession(manifest: WorkflowSessionManifest, extra: number): Promise<WorkflowSessionManifest> {
    ...
    while (current.heads.length + extra > this.options.maxRetainedRunsPerSession) {
      ...
      if (!victim) throw new BoundedFileError(`workflow Session "${manifest.sessionId}" cannot retain another run`, 'WORKFLOW_STORAGE_LIMIT')
      current = { ...current, revision: current.revision + 1, heads: current.heads.filter(head => head.runId !== victim.runId) }
      await this.persist(current)
      await this.removeRunTree(victim.runDirectory).catch(error => { if ((error as NodeJS.ErrnoException).code !== 'ENOENT') throw error })
      this.details.delete(victim.runId)
    }
    return current
  }
  private async enforceGlobalLimit(signal?: AbortSignal): Promise<void> {
    ...
    while (bytes > this.options.maxRunStoreBytes) {
      ...
      if (!victim) throw new BoundedFileError(`workflow run store exceeds the ${this.options.maxRunStoreBytes}-byte limit`, 'WORKFLOW_STORAGE_LIMIT')
      ...
      await this.removeRunTree(victim.head.runDirectory, signal); this.details.delete(victim.head.runId)
   ...
   * staging.
   */
  private async preflightTransactionCapacity(
    previous: WorkflowSessionManifest,
    candidate: WorkflowSessionManifest,
    initialFileBytes: number,
    signal?: AbortSignal,
  ): Promise<WorkflowSessionManifest> {
    ...
    for (;;) {
      ...
      if (projectedPeak <= this.options.maxRunStoreBytes) return next
      ...
      const victim = [...this.sessions.values()]
        ...
        .sort((left, right) =>
          (left.head.settledAt ?? 0) - (right.head.settledAt ?? 0)
          || left.head.startedAt - right.head.startedAt
          || left.head.runId.localeCompare(right.head.runId),
        )[0]
      if (victim === undefined) {
      ...
      const reduced: WorkflowSessionManifest = {
        ...victim.manifest,
  ...
  async insertWithNextDisplayName(request: WorkflowRunInsertRequest, create: (identity: WorkflowRunIdentity) => { readonly head: any; readonly detail: WorkflowRunDetailPayloadV2 }, signal?: AbortSignal): Promise<WorkflowRunHeadRecord> {
    return this.mutate(async () => {
      await this.guard(signal); await this.ensureInitialized(signal)
      if (!isWorkflowDefinitionName(request.name)) throw new BoundedFileError(`invalid workflow name "${request.name}"`, 'WORKFLOW_STORAGE_CORRUPT')
      const scriptBytes = encoder.encode(request.script)
      if (scriptBytes.byteLength > 1_048_576) throw new BoundedFileError('workflow script exceeds 1048576 bytes', 'WORKFLOW_STORAGE_LIMIT')
      let manifest = await this.load(request.sessionId)
      const ordinalEntry = manifest.ordinals.find(item => item.name === request.name)
      if (!ordinalEntry && manifest.ordinals.length >= this.options.maxWorkflowNamesPerSession) throw new BoundedFileError(`workflow Session has reached the ${this.options.maxWorkflowNamesPerSession}-name limit`, 'WORKFLOW_STORAGE_LIMIT')
      ...
      const runDirectory = id(); const identity = { displayName, numberedHandle, runDirectory }
      const draft = create(identity); const detailId = id()
      const detail: WorkflowRunDetailSnapshotV2 = { version: 2, sessionId: request.sessionId, runId: request.runId, runDirectory, detailId, snapshotRevision: 1, payload: compactWorkflowRunDetails(draft.detail, detailLimits(this.options)) }
      ...
      const stageId = id()
      const stage = join(this.options.runsRoot, 'staging', stageId); const run = this.runPath(runDirectory)
      try {
        if (this.layout !== undefined) {
          ...
          await this.layout.staging.assertIdentity(signal); await this.layout.runs.assertIdentity(signal)
        } else {
          await ownerDirectory(stage, true)
          await ownerDirectory(join(stage, 'scratch'), true); await ownerDirectory(join(stage, 'details'), true)
        ...
        if (this.layout !== undefined) {
          ...
          if (!current?.heads.some(candidate => candidate.runId === request.runId)) await this.removeRunTree(runDirectory).catch(() => undefined)
        } else {
          await safeRemoveTree(stage).catch(() => undefined); await safeRemoveTree(run).catch(() => undefined)
        }
        throw error
      }
    })
  ...
  async commitRun(request: WorkflowRunCommitRequest, signal?: AbortSignal): Promise<WorkflowRunHeadRecord> {
    return this.mutate(async () => {
      await this.guard(signal); await this.ensureInitialized(signal)
      const manifest = await this.load(request.sessionId); const index = manifest.heads.findIndex(head => head.runId === request.runId)
      if (index < 0) throw new BoundedFileError('workflow run not found', 'WORKFLOW_RUN_NOT_FOUND')
      const old = manifest.heads[index]
      if (old.revision !== request.expectedRevision) throw new BoundedFileError('workflow run changed; refresh it before applying a control', 'WORKFLOW_STALE_REVISION')
      if (terminal(request.head.status)) throw new BoundedFileError('terminal updates must claim a completion notice', 'WORKFLOW_STORAGE_CORRUPT')
      let reference = old.detail
      let replacement: WorkflowRunDetailSnapshotV2 | undefined
      let replacementBytes: Uint8Array | undefined
      ...
      try {
        if (replacement !== undefined && replacementBytes !== undefined) {
          if (this.layout !== undefined) await this.writeCapabilityFile(['runs', old.runDirectory, 'details'], `${replacement.detailId}.json`, new TextDecoder().decode(replacementBytes), this.options.maxRunDetailsBytes, true)
          else await writeBoundedAtomic(join(this.runPath(old.runDirectory), 'details', `${replacement.detailId}.json`), new TextDecoder().decode(replacementBytes), { maxBytes: this.options.maxRunDetailsBytes, createOnly: true })
          replacementWritten = true
        }
        await this.persist(fitting)
      } catch (error) {
        if (replacementWritten && replacement !== undefined) await this.removeCapabilityFile(['runs', old.runDirectory, 'details'], `${replacement.detailId}.json`).catch(() => undefined)
        throw error
      }
  ...
  async commitTerminalAndClaimNotice(request: WorkflowTerminalCommitRequest, signal?: AbortSignal): Promise<WorkflowRunHeadRecord> {
    return this.mutate(async () => {
      await this.guard(signal); await this.ensureInitialized(signal)
      const manifest = await this.load(request.sessionId); const index = manifest.heads.findIndex(head => head.runId === request.runId)
      if (index < 0) throw new BoundedFileError('workflow run not found', 'WORKFLOW_RUN_NOT_FOUND')
      const old = manifest.heads[index]
      if (old.revision !== request.expectedRevision) throw new BoundedFileError('workflow run changed; refresh it before applying a control', 'WORKFLOW_STALE_REVISION')
      if (!terminal(request.head.status)) throw new BoundedFileError('terminal commit requires a terminal status', 'WORKFLOW_STORAGE_CORRUPT')
      let reference = old.detail
      let replacement: WorkflowRunDetailSnapshotV2 | undefined
      let replacementBytes: Uint8Array | undefined
      if (request.detail !== undefined) {
      ...
      try {
        if (replacement !== undefined && replacementBytes !== undefined) {
          if (this.layout !== undefined) await this.writeCapabilityFile(['runs', old.runDirectory, 'details'], `${replacement.detailId}.json`, new TextDecoder().decode(replacementBytes), this.options.maxRunDetailsBytes, true)
          else await writeBoundedAtomic(
            join(this.runPath(old.runDirectory), 'details', `${replacement.detailId}.json`),
            new TextDecoder().decode(replacementBytes),
            { maxBytes: this.options.maxRunDetailsBytes, createOnly: true },
          )
          replacementWritten = true
        }
        await this.persist(fitting)
  ...
  async finalizeCompletionNotice(sessionId: string, runId: string, expectedRevision: number, finalization: WorkflowCompletionNoticeFinalization, signal?: AbortSignal): Promise<WorkflowRunHeadRecord> {
    return this.mutate(async () => {
      await this.guard(signal); await this.ensureInitialized(signal)
      const manifest = await this.load(sessionId); const index = manifest.heads.findIndex(head => head.runId === runId)
      if (index < 0) throw new BoundedFileError('workflow run not found', 'WORKFLOW_RUN_NOT_FOUND')
      const old = manifest.heads[index]
      if (old.revision !== expectedRevision) throw new BoundedFileError('workflow run changed; refresh it before applying a control', 'WORKFLOW_STALE_REVISION')
      if (old.completionNotice.state !== 'claimed') throw new BoundedFileError('completion notice is not claimable', 'WORKFLOW_STORAGE_CORRUPT')
      if (finalization.claimId !== old.completionNotice.claimId || finalization.processEpoch !== old.completionNotice.processEpoch || finalization.claimedAt !== old.completionNotice.claimedAt || finalization.finalizedAt < finalization.claimedAt) throw new BoundedFileError('completion notice claim does not match its owner', 'WORKFLOW_STALE_REVISION')
  ...
  async readSession(sessionId: string, signal?: AbortSignal): Promise<readonly WorkflowRunHeadRecord[]> {
  ...
  async readDetails(runId: string, request: DetailReadRequest, signal?: AbortSignal): Promise<DetailReadResult> {
    return this.mutate(async () => {
      await this.guard(signal); await this.ensureInitialized(signal)
      let found: { head: WorkflowRunHeadRecord; sessionId: string } | undefined
      ...
      if (request.cursor !== undefined) {
        if (!/^\d+$/u.test(request.cursor)) throw new BoundedFileError('workflow detail cursor is invalid', 'WORKFLOW_STORAGE_CORRUPT')
        start = Number(request.cursor)
        if (!Number.isSafeInteger(start) || start < 0 || (collection && start > raw.length)) throw new BoundedFileError('workflow detail cursor is invalid', 'WORKFLOW_STORAGE_CORRUPT')
      }
      const limit = request.limit ?? 200
      if (!Number.isSafeInteger(limit) || limit < 1 || limit > 200) throw new BoundedFileError('workflow detail page limit is invalid', 'WORKFLOW_STORAGE_LIMIT')
  ...
  async dispose(): Promise<void> { if (this.disposed) return; await this.mutationTail; this.disposed = true; this.sessions.clear(); this.details.clear() }
}
