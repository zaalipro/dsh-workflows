...
export interface WorkflowRunInsertRequest {readonly sessionId:string;readonly name:string;readonly runId:string;readonly script:string;readonly description?:string;readonly args?:JsonValue;readonly budgetTotal?:number}
...
export interface WorkflowTerminalCommitRequest extends Omit<WorkflowRunCommitRequest,'head'>{readonly head:Omit<WorkflowRunHeadRecord,'detail'|'completionNotice'>}
export interface DetailReadRequest {readonly kind:'members'|'logs'|'result'|'artifacts'|'artifact';readonly cursor?:string;readonly name?:string;readonly maxBytes?:number}
export interface DetailReadResult {readonly value:JsonValue;readonly revision:number;readonly nextCursor?:string}
