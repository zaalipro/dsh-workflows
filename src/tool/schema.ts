...
function detachedRecord(value: Record<string, unknown>, field: string): Record<string, unknown> {
  ...
  try { snapshot = snapshotWorkflowJsonValue(value) } catch (cause) {
    throw new Error(`workflow ${field} must contain lossless JSON data`, { cause })
  }
  if (!record(snapshot)) throw new Error(`workflow ${field} must be a JSON object`)
  return snapshot
}
...
export function parseWorkflowToolRequest(value:unknown):ParsedWorkflowToolRequest {
  ...
  if(sources.length!==1) throw new Error('workflow requires exactly one source: name, script, or script_path')
