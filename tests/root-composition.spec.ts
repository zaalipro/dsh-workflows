      ...
      return new Proxy(original, {
        apply(target, thisArg, args: [string]) {
          if (args[0] === '@deepseek-ai/dsh/package.json') {
            ...
            return { name: '@deepseek-ai/dsh', version: process.env.DSH_TEST_HOST_VERSION ?? '0.1.6-alpha.1' }
          }
          if (args[0] === '@deepseek-ai/dsh-workflow-ptc/package.json') return { version: '0.1.6-alpha.1' }
          if (args[0] === '@deepseek-ai/dsh-workflow/package.json') return {
            name: '@deepseek-ai/dsh-workflow',
            version: process.env.DSH_TEST_WORKFLOW_VERSION ?? '0.1.6-alpha.1',
          }
...
describe('package identity and Host inject', () => {
  it('exports the package row identity without blocking boot on optional Remote events', () => {
    ...
    expect(version).toBe('0.1.0-rc.11')
...
describe('exact official Host compatibility', () => {
  it('mirrors the manifest contract and rejects every unverified version', () => {
    expect(isSupportedHostVersion('0.1.6-alpha.1')).toBe(true)
    for (const value of [undefined, null, 1, 'future', '0.1.0', '0.1.1', '0.1.1-rc.3', '0.1.5-rc.1', '0.1.5-rc.2', '0.1.6-alpha.2']) {
      expect(isSupportedHostVersion(value)).toBe(false)
    }
    expect(isSupportedHostVersions('0.1.6-alpha.1', '0.1.6-alpha.1')).toBe(true)
    expect(isSupportedStockHost()).toBe(true)
    expect(() => assertSupportedHostVersions('0.1.6-alpha.1', '0.1.6-alpha.1')).not.toThrow()
  })

  it('requires both the CLI host and evaluator seam to have the exact version', () => {
    for (const versions of [
      ['future', '0.1.6-alpha.1'],
      ['0.1.5-rc.1', '0.1.6-alpha.1'],
      ['0.1.6-alpha.1', '0.1.5-rc.1'],
      ['0.1.6-alpha.1', '0.1.6-alpha.2'],
      ['0.1.6-alpha.1', 'future'],
      ['0.1.1-rc.0', '0.1.6-alpha.1'],
      ['0.1.6-alpha.1', '0.1.1-rc.3'],
    ] as const) {
      ...
      expect(() => assertSupportedHostVersions(...versions)).toThrowError(expect.objectContaining({
        ...
        message: '@zaalipro/dsh-workflows 0.1.0-rc.11 supports exactly official DeepSeek Harness 0.1.6-alpha.1',
      }))
    }
  })
