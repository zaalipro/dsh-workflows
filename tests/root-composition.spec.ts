      ...
      return new Proxy(original, {
        apply(target, thisArg, args: [string]) {
          if (args[0] === '@deepseek-ai/dsh/package.json') {
            ...
            return { name: '@deepseek-ai/dsh', version: process.env.DSH_TEST_HOST_VERSION ?? '0.1.5-rc.1' }
          }
          if (args[0] === '@deepseek-ai/dsh-workflow-worker-thread/package.json') return { version: '0.1.5-rc.1' }
          if (args[0] === '@deepseek-ai/dsh-workflow/package.json') return {
            name: '@deepseek-ai/dsh-workflow',
            version: process.env.DSH_TEST_WORKFLOW_VERSION ?? '0.1.5-rc.1',
          }
...
describe('exact official Host compatibility', () => {
  it('mirrors the manifest contract and rejects every unverified version', () => {
    expect(isSupportedHostVersion('0.1.5-rc.1')).toBe(true)
    for (const value of [undefined, null, 1, 'future', '0.1.0', '0.1.1', '0.1.1-rc.3']) {
      expect(isSupportedHostVersion(value)).toBe(false)
    }
    expect(isSupportedHostVersions('0.1.5-rc.1', '0.1.5-rc.1')).toBe(true)
    expect(isSupportedStockHost()).toBe(true)
    expect(() => assertSupportedHostVersions('0.1.5-rc.1', '0.1.5-rc.1')).not.toThrow()
  })
  ...
  it('requires both the CLI host and evaluator seam to have the exact version', () => {
    for (const versions of [
      ['future', '0.1.5-rc.1'],
      ['0.1.5-rc.1', 'future'],
      ['0.1.1-rc.0', '0.1.5-rc.1'],
      ['0.1.5-rc.1', '0.1.1-rc.3'],
    ] as const) {
      ...
      expect(() => assertSupportedHostVersions(...versions)).toThrowError(expect.objectContaining({
        ...
        message: '@zaalipro/dsh-workflows 0.1.0-rc.6 supports exactly official DeepSeek Harness 0.1.5-rc.1',
      }))
    }
  })
