...
describe('package policy verifier', () => {
  ...
  it('rejects an unverified Harness peer range or compatibility declaration', () => withFixture(fixture => {
    const manifestPath = join(fixture, 'package.json')
    const manifest = JSON.parse(readFileSync(manifestPath, 'utf8'))
    manifest.peerDependencies['@deepseek-ai/dsh-workflow'] = '>=0.1.5-rc.1'
    manifest.dsh.compatibility.versions = ['0.1.5-rc.1', '0.1.1']
    writeFileSync(manifestPath, `${JSON.stringify(manifest, null, 2)}\n`)
    expectFailure('--source', fixture)
  }))
...
function makeManifest() {
  ...
  const peers = {
    '@deepseek-ai/cordis': '>=4.0.2', '@deepseek-ai/dsh-client-connection': '0.1.5-rc.1',
    '@deepseek-ai/dsh-client-ui-conversation': '0.1.5-rc.1',
    '@deepseek-ai/dsh-workflow': '0.1.5-rc.1', '@deepseek-ai/dsh-workflow-worker-thread': '0.1.5-rc.1', react: '>=18',
  }
  return {
    name: '@zaalipro/dsh-workflows', version: '0.1.0-rc.6', type: 'module', license: 'MIT',
    ...
    devDependencies: { ...peers, '@deepseek-ai/dsh': '0.1.5-rc.1' },
    dsh: { compatibility: { host: '@deepseek-ai/dsh', versions: ['0.1.5-rc.1'], evaluator: 'plugin-compat-engine-v1' }, bundle: { patch: './cordis.patch.yml' }, client: { platform: 'web', inject: ['@deepseek-ai/dsh-client-connection', '@deepseek-ai/dsh-client-ui-conversation'] } },
  }
}
